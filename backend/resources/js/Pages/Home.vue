<template>
    <app-layout>
        <div class="bg-gradient-to-br from-pink-700 to-yellow-400 relative flex items-center justify-center" style="height: 93vh;">
            <div class="absolute top-0 left-0 h-full w-full bg-cover bg-app-top opacity-10"></div>
            <div class="text-white left-6 z-10 flex flex-col items-center justify-center">
                <h1 class="text-7xl">{{ __('No more hesitation') }}</h1>
                <h3 class="text-3xl">{{ __('Accelerate team building with DevCheers') }}</h3>
                <p>{{ __('DevCheers is team building tool designed for a little or no experienced junior developers.') }}</p>
                <p>{{ __('You can create your own team or join other teams and manage them easily with convenient features.') }}</p>
                <div class="text-2xl my-4"><span @click.stop="showGetStartedModalAndOpener" :class="{ 'hover:text-green-200 text-green-300 cursor-pointer': ! isGetStartedUnabled, 'cursor-not-allowed text-gray-400': isGetStartedUnabled }">{{ __('Get started') }}</span></div>
            ...
            <div v-if="showOpener===true" @click.stop="openGetStartedModal=true" class="cursor-pointer absolute top-4 right-4 opacity-50 hover:opacity-100 bg-blue-400 text-white rounded-md border border-gray-200 h-28 w-40">
    ...
    export default {
        ...
        data() {
            return {
                ...
                isGetStartedUnabled: false
            }
        },

        watch: {
            isJustLoggedOut() {
                if(this.isJustLoggedOut) {
                    this.openGetStartedModal = false;
        ...
        methods: {
            showGetStartedModalAndOpener() {
                if(!this.isGetStartedUnabled) {
                    this.openGetStartedModal = true;
        ...
        created() {
            if(this.fromSetup.isLoadedFromCache) {
                ...
                axios.get(route('get-started-modal-session-data')
                    ...
                    this.isGetStartedUnabled = false;
                });
            }
